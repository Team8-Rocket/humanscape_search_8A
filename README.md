...
## 구현하면서 어려웠던 점
### api 요청 
결과에 따라 데이터 형식이 달라서 조건문을 많이 사용했는데 더 좋은 방법을 찾기 어려웠습니다.  
api 호출 최적화를 위해 cancel 토큰을 다양하게 사용해보려 했으나 크게 효용성이 없는 것 같아 삭제했습니다.
### API 호출 
횟수를 제한하는 방식을 어떻게 구현하는 방식에 대한 고민이 있었고 디바운싱과 axios 캔슬 토큰을 활용하였다.
### 키보드 이벤트 
키보드 이벤트 진행시 처음에 두번 튀는 에러가 있었고 event.nativeEvent.isComposinng로 에러를 해결하였다. 그리고 handleKeyMove 함수에 if/ else if가 너무 많이들어가 코드를 축소하고, 가독성이 더 좋게 만드는 방식을 생각하는게 어려웠다.
### 마크업
코드 구성을 할 때 태그를 의미적으로 통하게 만드는것도 은근 어려웠다. 의미적으로 구분이 되어있으니 조금 더 딥하게 들어가게되는것 같다. 팀원분들과 상의 mdn사이트로 조금 더 정확하게 태그를 구성하려고 노력하였다.
### 추천검색어
결과 화면 외부영역 클릭시 닫기를 구현할 때 useRef훅을 이용하여 구현하였으나 어떤 DOM을 ref로 지정해야 하는지에 대한 어려움이 있었고 css를 이용하여 해결하였다.
### 퍼지 문자열 검색
json 형식으로 데이터를 가지고 있을때를 가정할 경우 퍼지 검색기능을 구현하였다.
정규 표현식으로 초성만 있을경우 , 초성+중성 , 초성+중성+종성 모두 있을경우를 처리하였다. 하지만 api를 호출하여 데이터를 받으면서 퍼지문자검색을 하는 것은 구현하지 못하였다. 
